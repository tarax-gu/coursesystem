...
istream &read_course(istream &is, course &item )
{
    ...
    item.homework_filename = "_" + item.homework_filename + ".txt"; 
	ifstream in;
    ...
    item.course_data_filename = "_" + item.course_data_filename + ".txt";
...
ostream &print_course(ostream &os,course &item)
{
    ...
    {
    	if(item.course_work[i].now == 1){
    		os << "作业名称：" << item.course_work[i].workname << " " << "提交状态：";
